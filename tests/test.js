QUnit.test("picolog", function( assert ) {
	...
	assert.ok(log.TRACE === 1, '`log.TRACE` exists and is 1');
	assert.ok(log.DEBUG === 2, '`log.DEBUG` exists and is 2');
	assert.ok(log.INFO === 3, '`log.INFO` exists and is 3');
	assert.ok(log.WARN === 4, '`log.WARN` exists and is 4');
	assert.ok(log.ERROR === 5, '`log.ERROR` exists and is 5');
	assert.ok(log.NONE === 9, '`log.NONE` exists and is 9');
